...
- [ ] 🔴 **SQL Server → Postgres migration** `launch/postgres-migration` — decided 2026-09-05. Provider swap, not a data migration: it happens before launch so there is no production data, no dual-write window and no ETL, and each service owns its own database so the cut-over is per service rather than a flag day. Phases 1-2 are delivered: provider-neutral string configuration and the shared cross-provider `geography` seam are published and consumed with unchanged SQL Server schemas. Phase 3 removes the remaining test-only `SET IDENTITY_INSERT` and `sys.check_constraints` SQL. Measured surface at planning time: 8 identity blocks, 2 constraint queries, 8 spatial columns, 5 string column types, the B2B-local `IsRowVersion()` token with 5 implementers, and 50 migration files. Driven by PostGIS, `EXCLUDE` constraints for venue double-booking, and Testcontainers startup cost — not by a SQL Server capability ceiling. Plan: [POSTGRES_MIGRATION_PLAN.md](POSTGRES_MIGRATION_PLAN.md).
...
- [ ] 🟡 **Versioned configurable Deal model** `launch/deal-configuration` — selected 2026-09-10: relational ownership, revisions and capability selections plus a strongly validated `jsonb` term graph and complete accepted Contract snapshot. The four launch arrangements become platform-supplied presets of one finite C# rule/capability language; no permanent four-built-ins-plus-Composite split. Typed-language work does not wait for .NET 11; persistence delivery follows the B2B Postgres cut-over. The representation refactor is not an additional MVP gate; tenant authoring, tenant-specific entitlements and a visual builder are separate future scope, with no Finbuckle dependency. Plan: [DEAL_CONFIGURATION_PLAN.md](DEAL_CONFIGURATION_PLAN.md).
  Future richer-rule discovery for mid-sized events and the open tenant-policy ownership/tenancy-infrastructure decisions are captured in that plan, outside MVP and this refactor's acceptance gates. Finbuckle is not ruled out for that future work.
