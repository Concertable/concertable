...
9. **Transactional outbox** in each service's own DB. **Library DONE at `86b9b6f7` (2026-05-20):** parallel agent shipped `Concertable.Messaging`'s outbox layer — `OutboxBus` wraps `IBusTransport` publishes into `OutboxMessageEntity` rows, `OutboxDispatcher` (BackgroundService) drains them through the underlying transport, `OutboxStore` + `OutboxMessageEntityConfiguration` handle persistence, `OutboxOptions` controls polling/batch sizing, `AddMessagingOutbox` extension wires the lot. Same commit relocated `MessageSerializer`/`MessageTypeRegistry`/`EventRegistrationExtensions` from `Messaging.AzureServiceBus` → `Messaging.Application` (transport-agnostic primitives belong in Application; ASB stays a thin adapter). New `Concertable.Messaging.UnitTests` project covers MessageEnvelope/Serializer/Registry/OutboxBus/OutboxDispatcher/OutboxStore/OutboxMessageEntity. **Still to do at the per-service level:** add `OutboxMessageEntity` to each service's `DbContext`, add the outbox migration, register `AddMessagingOutbox<TDbContext>` in each composition root. Solves the dual-write problem (§6 callout).
