...
14. **Extract `Concertable.Payment.Api` + `Concertable.Payment.Workers`** to its own host + own DB + own Stripe webhook endpoint. PCI scope shrinks dramatically. Stripe webhook URL change in dashboard. **IN PROGRESS 2026-05-20.** Steps 1–4 done: comms doc corrected, projects restructured to `Concertable.Payment/`, `Payment.Contracts` phased out (types redistributed to Domain/Application.DTOs/Payment.Client), `Payment.Client` classlib created with gRPC adapters (`IXClient`/`XClient` naming). Remaining: Payment.Web, Payment.Workers, StripeAccountController cleanup, B2B/Customer/Workers/AppHost wiring. Plan at `.claude/plans/shiny-strolling-alpaca.md`.
